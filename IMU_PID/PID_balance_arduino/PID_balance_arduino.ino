...
void setup() {
  ...
  timeCurrent = millis(); //Start counting timeCurrent in milliseconds
  ...
  delay(1000); /*Give some delay, 7s, to have timeCurrent to connect
...
/////////////////////////////I M U/////////////////////////////////////
    timePrev = timeCurrent;  // the previous timeCurrent is stored before the actual timeCurrent read
    timeCurrent = millis();  // actual timeCurrent read
    elapsedTime = (timeCurrent - timePrev) / 1000; 
  
  /*The tiemStep is the timeCurrent that elapsed since the previous loop. 
  ...
  // timeCurrent divided by that timeCurrent. For taht we will use a variable called previous_error.
  // We substract that value from the actual error and divide all by the elapsed timeCurrent. 
